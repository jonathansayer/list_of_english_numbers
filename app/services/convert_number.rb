...
@teens = { '11' => 'eleven',
  ...
  def self.convert number
    ...
    self.hundreds_of_thousands number if number.length > 4
  end
  ...
  def self.thousands number
    ...
    @numbers[number[0]] + ' thousand and ' + self.hundreds(number[1] + number[2] + number[3])
  end
  ...
  def self.hundreds number
    ...
    @numbers[number[0]] + ' hundred and ' + self.tens(number[1] + number[2])
  end

  def self.tens number
    return @tens[number[0]] if number.length == 2 and number[1] == '0'
    return @teens[number] if self.is_teen? number
    (@tens[number[0]] + " " + @numbers[number[1]]) if number.length == 2 and !self.is_teen? number
  end
